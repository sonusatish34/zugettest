        ...
        {slides.map((slide, index) => (
          <SwiperSlide key={slide.id}>
            <div className="relative w-full h-full">
              {/* <Image
                src={slide.url}
                alt={slide.title || 'Banner image'} // Added a fallback for slides without titles
                fill
                priority={index === 0}
                className="object-cover object-center" // FIX: Applied object-cover to all sizes, centered it
              /> */}
              <Image
